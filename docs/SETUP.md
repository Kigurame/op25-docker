...
| `GET /playlist.m3u` | M3U playlist of every enabled mount for players like Jellyfin. GET + HEAD (Jellyfin probes with HEAD). Auth when `listener_auth` is on: web session, HTTP Basic, **or** `?user=&password=` query params matching a listener account — players like Jellyfin never send Basic auth for tuner URLs. Stream URLs embed the listener login when auth is on and use the request host + `icecast.external_port` |
